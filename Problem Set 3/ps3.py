...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        # make tiles (as a dictionary)
        self.tiles = {}
        for i in range(math.floor(width)):
            for j in range(math.floor(height)):
    ...
    def get_num_cleaned_tiles(self):
        ...
        for tile in self.tiles:
            if math.floor(self.tiles[tile]) == 0:
                counter += 1
        return counter
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_pos = old_pos.get_new_position(self.get_robot_direction(), self.speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        cleaned_tiles = 0
        clock_tick = 0
        while (cleaned_tiles / num_tiles) < min_coverage:
            clock_tick += 1
            for rob in robots:
                rob.update_position_and_clean()
                #anim.update(room, robots)
            cleaned_tiles = room.get_num_cleaned_tiles()
        time_steps.append(clock_tick)  
        #anim.done()
    return statistics.mean(time_steps)   
